...
\begin{document}
	...
	\begin{itemize}
		\item \textbf{Opis działania:} Plik \textbf{\texttt{supervisor.c}} implementuje orkiestrację procesu nadzorczego. Na podstawie ilości argumentów, tworzone są procesy robocze, czyli workery, gdzie ich implementacja jest w \textbf{\texttt{worker.c}}, gdzie każdy odpowiada za jeden cykl skanowania dla konkretnego wzorca. Stan procesów (cykl życia, PID) jest rejestrowany i utrzymywany w strukturach modułu \textbf{\texttt{proc\_table.c}}, co ułatwia zarządzanie i ewentualne zabijanie/restartowanie workerów.
	...
	\begin{itemize}
		\item \textbf{Opis działania:} \textbf{\texttt{signals.c}} konfiguruje handlery dla sygnałów \textbf{\texttt{SIGUSR1}} (budzenie/restart skanowania) i \textbf{\texttt{SIGUSR2}} (przerwanie skanowania i uśpienie). Zmieniają one globalne flagi kontrolne. Z kolei \textbf{\texttt{sleep\_control.c}} używa tych flag do zarządzania interwałami snu nadzorcy i jego przedwczesnym wybudzaniem.
		\item \textbf{Kluczowe zmienne globalne:} Atomowe flagi systemowe (\textbf{\texttt{volatile sig\_atomic\_t}}), na których opiera się asynchroniczne przerywanie funkcji np. \textbf{\texttt{sleep()}}, powiadamiające demona, czy ma natychmiast przestać szukać czy zrestartować skanowanie.
	\end{itemize}
	
	\section{Zrzuty ekranu działającego programu (demonstracja)}
	
	W celu zaprezentowania poprawności działania programu załączono następujące scenariusze:
	
	\begin{enumerate}
		\item Standardowe uruchomienie z wieloma wzorcami - widok współbieżnego dopasowania plików do poszczególnych argumentów w \textbf{\texttt{/var/log/syslog}}.
		\item Przebieg z flagą \textbf{\texttt{-v}} pokazujący zdarzenia: uśpienie, wybudzenie, sygnały i porównywanie ścieżek.
		\item Obsługa sygnału \textbf{\texttt{SIGUSR1}} wysłanego do demona w trakcie snu (natychmiastowe obudzenie).
		\item Obsługa sygnałów w trakcie pracy - symulacja wysłania \textbf{\texttt{SIGUSR1}} do pracującego demona w celu zrestartowania pętli, a następnie \textbf{\texttt{SIGUSR2}} w celu przerwania skanu i uśpienia całego procesu.
	\end{enumerate}
